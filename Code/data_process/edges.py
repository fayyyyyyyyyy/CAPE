...
for project_name in project_names:
    ...
    output_file = filepath + dataset_keyDesign_root + project_name + '/edges.txt'
    ...
    with open(map_file, 'r') as ant_file, open(output_file, 'w') as edges_file:
        ...
        for line in ant_file:
            i = i + 1
            class1, class2, _ = line.strip().split('\t')
            if class1 in nodes_dict and class2 in nodes_dict:
                edges_file.write(f"{nodes_dict[class1]}\t{nodes_dict[class2]}\n")
                # print(f"{nodes_dict[class1]}\t{nodes_dict[class2]}")
